...
# ── Utility: resilient GET with hyphen-fallback ──────────────────────────────
def resilient_get(url_template: str, execution_id: str, **kwargs) -> requests.Response:
    """
    Try the request with the original execution_id first.
    If the server returns a 4xx error (bad URL / not found), retry once
    with hyphens removed from the execution_id.
    """
    for eid in _id_variants(execution_id):
        url = url_template.format(execution_id=eid)
        resp = requests.get(url, headers=HEADERS, **kwargs)
        if resp.status_code < 400:
            return resp
        print(f"  ⚠️  Got {resp.status_code} for ID '{eid}', trying next variant...")

    # All variants failed — raise on the last response so the caller sees the error
    resp.raise_for_status()
    return resp  # unreachable, but satisfies type checkers


def _id_variants(execution_id: str) -> list[str]:
    """Return [original, no-hyphens] deduplicated."""
    stripped = execution_id.replace("-", "")
    if stripped == execution_id:
        return [execution_id]          # nothing to strip
    return [execution_id, stripped]
...
def download_artifacts_zip(execution_id: str) -> Path:
    ...
    # Use whichever ID variant actually worked (reflected in the final URL)
...
def run_pipeline(execution_id: str):
    print("=" * 55)
